...
class EasyOCRService(BaseOCRService):
    ...
    async def process_file(self, file_bytes: bytes, filename: str, content_type: str) -> Tuple[str, Optional[float], Optional[Dict[str, Any]]]:
        """Process raw file with EasyOCR service using layout-parsing protocol"""
        ...
        if result.get('errorCode') != 0:
            raise Exception(f"EasyOCR processing failed: {result.get('errorMsg', 'Unknown error')}")
    ...
    async def health_check(self) -> bool:
        """Check if EasyOCR service is healthy"""
        try:
