"""PP-OCRv4 Mobile service client"""
import httpx
import base64
...
class PPOCRv4MobileService(BaseOCRService):
    """PP-OCRv4 Mobile service client"""
    ...
    async def process_file(self, file_bytes: bytes, filename: str, content_type: str) -> Tuple[str, Optional[float], Optional[Dict[str, Any]]]:
        """Process raw file with PP-OCRv4 Mobile service using layout-parsing protocol"""
        ...
        if result.get('errorCode') != 0:
            raise Exception(f"PP-OCRv4 Mobile processing failed: {result.get('errorMsg', 'Unknown error')}")
    ...
    async def health_check(self) -> bool:
        """Check if PP-OCRv4 Mobile service is healthy"""
        try:
